...
import com.blink.web.meta.handlers.ArticleTagHandler;
...
public class BlinkVerticle extends AbstractVerticle {
    ...
    public BlinkVerticle(Context context) throws Exception {
        ...
        this.metaTagResolver.register("/articles/:key").handler(new ArticleTagHandler(context));
    }
    ...
    private void routeAPIRequest(RoutingContext context) {
        ...
        if (allowedOrigins.isEmpty() || allowedOrigins.contains(host))
            worker.publishRequest(params.get("target"), params.get("targetUser"), appKey, appSession, params.get("message"), origin, context.request().remoteAddress().host(), context.response());
        else {
